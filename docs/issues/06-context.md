...
Implement `agentflow context --step <name>`. This is the most complex command — it assembles everything the agent needs to complete a step without exploring the project itself.
...
- Implement `src/commands/context.ts` and any supporting assembly logic (may live in a `src/context/` module or inline):
  ...
  - The final line of context output is always the exact `agentflow complete --step <name>` command.
...
- [x] `agentflow context --step <name>` outputs the instruction file content
- [x] Reference files are inlined
- [x] Upstream step outputs are inlined; missing required outputs produce a clear error; skipped optional steps are noted
- [x] Revision state includes the previous output and reviewer file
- [x] `validates` steps output in the correct order (validated files → generates → evaluate instruction → command)
- [x] Every context output ends with the exact `agentflow complete` command
- [x] Integration tests cover `ready` and `revision` states
